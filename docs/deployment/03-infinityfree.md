# Deployment ke InfinityFree — komika.infinityfreeapp.com
...
```
komika.infinityfreeapp.com (public_html/)
...
2. **Buat domain** di InfinityFree panel → dapat `komika.infinityfreeapp.com`
3. **Buat MySQL database** di panel InfinityFree → catat:
   - Database name (format: `epiz_xxxxx_comika`)
   - Database user (format: `epiz_xxxxx`)
   - Database password
   - Database host (biasanya `sql.epizy.com` atau `127.0.0.1`)
...
2. Buka **File Manager** untuk domain `komika.infinityfreeapp.com`
...
# User: epiz_xxxxx (dari panel)
# Password: (dari panel)
# Port: 21

# Upload:
# - web/dist/* → /public_html/
# - backend/* → /public_html/app/
```
...
<IfModule mod_rewrite.c>
    ...
    # Uploaded files → Laravel storage
...
APP_DEBUG=false
APP_URL=https://komika.infinityfreeapp.com
FRONTEND_URL=https://komika.infinityfreeapp.com
...
DB_HOST=127.0.0.1
DB_PORT=3306
DB_DATABASE=epiz_xxxxx_comika
DB_USERNAME=epiz_xxxxx
DB_PASSWORD=password_kamu

CORS_ALLOWED_ORIGINS=https://komika.infinityfreeapp.com

TRUSTED_PROXIES=*
TRUSTED_HOSTS=komika.infinityfreeapp.com
...
MAIL_FROM_ADDRESS="noreply@komika.infinityfreeapp.com"
MAIL_FROM_NAME="COMIKA"

VAPID_SUBJECT=mailto:noreply@komika.infinityfreeapp.com
VAPID_PUBLIC_KEY=
VAPID_PRIVATE_KEY=
```
...
```bash
cd ~/domains/komika.infinityfreeapp.com/public_html/app
...
1. **Health check:**
   ```
   https://komika.infinityfreeapp.com/api/v1/health
   ```
   → Harus返回 JSON: `{"success":true,"message":"COMIKA API is running",...}`

2. **Frontend:**
   ```
   https://komika.infinityfreeapp.com/
   ```
   → Halaman utama COMIKA harus render

3. **SPA routing:**
   ```
   https://komika.infinityfreeapp.com/login
   https://komika.infinityfreeapp.com/discover
   ```
   → Tidak 404, harus tetap render React app

4. **API test:**
   ```
   https://komika.infinityfreeapp.com/api/v1/genres
   https://komika.infinityfreeapp.com/api/v1/comics
   ```
   → Return JSON data

5. **Storage test:**
   ```
   https://komika.infinityfreeapp.com/storage/comics/covers/comic-1.svg
   ```
...
| **CORS blocked** | `CORS_ALLOWED_ORIGINS` salah | Set ke `https://komika.infinityfreeapp.com` + `config:cache` |
| **Login gagal** | `SESSION_SECURE_COOKIE` false | Set `true` di `.env` |
| **Database connection** | Host/credentials salah | Cek panel InfinityFree untuk DB host yang benar |
...
- [ ] `https://komika.infinityfreeapp.com/` → frontend render
- [ ] `https://komika.infinityfreeapp.com/api/v1/health` → JSON response
- [ ] Login / Register bisa
