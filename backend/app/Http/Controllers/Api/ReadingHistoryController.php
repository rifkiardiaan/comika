...
{
    public function __construct(private readonly ReadingHistoryService $readingHistoryService)
    {
    }
    ...
    {
        $episode = Episode::findOrFail($request->integer('episode_id'));
        ...
        // Deteksi "baca baru" untuk gamification XP (sebelum recordProgress
        // yang meng-upsert baris riwayat).
        $isNewRead = ! ReadingHistory::where('user_id', $request->user()->id)
            ->where('episode_id', $episode->id)
            ->exists();

        // Pakai has() bukan filled(): filled(false) dan filled(0) bernilai false
        // sehingga nilai eksplisit 0 / false dari client tidak boleh diabaikan.
