...
class CreatorAnalyticsService
{
    ...
    public function dashboard(User $creator): array
    {
        ...
        // Laporan pembacaan: gratis vs berbayar
        $readingReport = $this->getReadingReport($comicIds);
    ...
    /**
     * Laporan pembacaan: gratis vs berbayar (dalam koin).
     *
     * @return array{free_reads: int, paid_reads: int, total_coins_from_paid: int, free_vs_paid_ratio: string}
     */
    private function getReadingReport($comicIds): array
    {
        if ($comicIds->isEmpty()) {
            return [
                'free_reads' => 0,
                'paid_reads' => 0,
                'total_coins_from_paid' => 0,
                'free_vs_paid_ratio' => '0:0',
            ];
        }

        // Hitung episode gratis yang dibaca (via reading_histories)
        $freeReads = ReadingHistory::whereIn('comic_id', $comicIds)
            ->whereHas('episode', fn ($q) => $q->where('is_premium', false))
            ->count();

        // Hitung episode berbayar yang sudah di-unlock
        $paidReads = EpisodeUnlock::whereHas('episode', fn ($q) => $q->whereIn('comic_id', $comicIds))
            ->count();

        // Total koin dari unlock berbayar
        $totalCoinsFromPaid = (int) EpisodeUnlock::whereHas('episode', fn ($q) => $q->whereIn('comic_id', $comicIds))
            ->sum('coins_spent');

        return [
            'free_reads' => $freeReads,
            'paid_reads' => $paidReads,
            'total_coins_from_paid' => $totalCoinsFromPaid,
            'free_vs_paid_ratio' => "{$freeReads}:{$paidReads}",
        ];
    }
}
