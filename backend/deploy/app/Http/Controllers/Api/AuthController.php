...
class AuthController extends Controller
{
    ...
    public function login(LoginRequest $request): JsonResponse
    {
        ...
        // Cek apakah akun diblokir — handle jika kolom belum ada
        try {
            if ($user->is_permanently_banned) {
                throw ValidationException::withMessages([
                    'email' => ['Akun ini telah diblokir permanen oleh admin. Hubungi dukungan untuk informasi lebih lanjut.'],
                ]);
            }
            if ($user->is_banned) {
                throw ValidationException::withMessages([
                    'email' => ['Akun ini sedang diblokir oleh admin. ' . ($user->ban_reason ?? '')],
                ]);
            }
        } catch (ValidationException $e) {
            throw $e;
        } catch (\Throwable $e) {
            // Kolom belum ada — abaikan, izinkan login
        }
